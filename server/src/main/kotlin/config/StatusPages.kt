...
//            call.respondText(text = "404: Cannot find relative url: ${call.request.uri}", status = code)
            call.respond(code, mapOf("message" to "Cannot find relative url: ${call.request.uri}"))
        }
...
//            call.respondText(text = "401: Not allowed to interact with relative url: ${call.request.uri}", status = code)
            call.respond(code, mapOf("message" to "Not allowed to interact with relative url: ${call.request.uri}"))
        }
        exception<Throwable> { call, cause ->
//            call.respondText(text = "500: $cause" , status = HttpStatusCode.InternalServerError)
            call.respond(HttpStatusCode.InternalServerError, mapOf("message" to cause.localizedMessage))
        }
    }
}
